...
import string
#----------------Integration Modules---------------
from On_69v4 import ProfessionalKeyboard
#from Virtual_Mouse_V10 import MainScreen
#from Virtual_Mouse_V10 import start_app
...
WOLFRAMALPHA_APP_ID = "LPQX8W-H378E22J9P"
...
def search_directory(folder_name):
    """Recursively search for the specified folder name in all drives."""
    # Get all drive letters available on the system
...
def open_directory(path):
    if os.path.exists(path):
        speak(f"Opening directory:")
        subprocess.Popen(f'explorer "{path}"')
    else:
        speak(f"Sorry, the directory {path} does not exist.")
...
def process_command(query, ui_queue):
    ...
    :return: None
    """
    if any(keyword in query for keyword in ['wikipedia', 'from wikipedia', 'from wikipedia tell me']):
        speak('Searching Wikipedia...')
        ui_queue.put(("system_output", "Searching Wikipedia..."))

        # Remove introductory phrases from the query
        for keyword in ['wikipedia', 'from wikipedia', 'from wikipedia tell me']:
   ...
   # elif "Using brain".lower() in query.lower():
    ...
    # -----------------------------------------------------------------------open ai----------------------
        ...
        ui_queue.put(("system_output", "Opening the music player..."))
        try:
          # Opens the default music player
            subprocess.Popen(["start", "wmplayer" , "/play"], shell=True)
        except Exception as e:
            speak("Unable to open the music player.")
            ui_queue.put(("system_output", "Unable to open the music player."))
        ...
        speak("Sorry! to know that you're not fine. Should I play music or Tell Any Joke?")
        ui_queue.put(("system_output", "sorry to know that you're not fine. Should I play music or Tell Any Joke?"))
        ...
        global bot_name
        bot_name = query.replace("change your name to", "").strip()
        ...
        """bot_name = take_command(q=None, ui_queue=ui_queue)"""
        ...
        try:
            ...
            client = wolframalpha.Client(WOLFRAMALPHA_APP_ID)  # Use the API key here
            ...
            ui_queue.put(("system_output", "I couldn't find an answer."))
        except Exception as e:
        ...
        import requests
        try:
            api_key = "ff084c24b87e4ff0a74f1f3cacfdcefc"
        ...
        try:
            ...
            logging.error(f"Error emptying Recycle Bin: {e}")  # Log the error for debugging

    elif "don't listen" in query or "stop listening" in query or "do not listen" in query or "stop talking" in query:
        try:
            match = re.search(r'(\d+)\s*(seconds?|minutes?)', query, re.IGNORECASE)
            if match:
                ...
                speak(f"Okay, I will not listen or speak for {duration} seconds.")
                ui_queue.put(("system_output", f"Will not listen for {duration} seconds."))

                 # Simulate stopping listening by waiting
                time.sleep(duration)

                 # Provide feedback that listening is resumed
                speak("Can I Speak again?")
                ui_queue.put(("system_output", "I am now listening again."))
            else:
                speak("Please specify the duration in seconds or minutes.")
                ui_queue.put(("system_output", "Please specify the duration in seconds or minutes."))
        except ValueError as ve:
            speak("There was an error with the value provided. Please specify a valid number.")
            ui_queue.put(("system_output", "Error with the value provided."))
            print(f"ValueError: {ve}")
        except Exception as e:
            speak("An unexpected error occurred.")
            ui_queue.put(("system_output", "An unexpected error occurred."))
            print(f"Exception: {e}")

    elif "locate" in query:
        query = query.replace("locate", "").strip()
        location = query
        ...
        try:
            ...
            if not cam.isOpened():
                ...
                ui_queue.put(("system_output", "Unable to open the camera."))
                return

            # Wait briefly to allow the camera to adjust (optional, for better focus)
            cam.set(cv2.CAP_PROP_BUFFERSIZE, 1)
            cv2.waitKey(500)  # Adjust timing if needed for your setup

            # Capture a single frame
            ret, frame = cam.read()
            if ret:
                cv2.imshow("Photo", frame)  # Show the captured photo briefly
                cv2.imwrite("img.jpg", frame)  # Save the photo
                speak("Photo taken and saved as img.jpg.")
                ui_queue.put(("system_output", "Photo taken and saved as img.jpg."))
                cv2.waitKey(1000)  # Display photo for 1 second before closing
            else:
                speak("Failed to capture an image.")
                ui_queue.put(("system_output", "Failed to capture an image."))
            ...
            if 'cam' in locals():  # Make sure the camera is released if open
                cam.release()
            cv2.destroyAllWindows()
            speak(f"Unable to access the camera: {e}")
            ui_queue.put(("system_output", f"Unable to access the camera: {e}"))
        ...
        try:
            import requests
            api_key = "cceafa9dcc41d852611ffb13b6fbbe48"
            base_url = "http://api.openweathermap.org/data/2.5/weather?"

            # Extract the city name by removing "what is the weather in"
            city_name = query.lower().replace("what is the weather in", "").strip()

            if city_name:
                # Complete URL with metric units for Celsius
                ...
                if x["cod"] != "404":
                    ...
                    # Formatted weather info in Celsius
                    ...
                    speak(f"City {city_name.capitalize()} not found. Please try again.")
                    ui_queue.put(("system_output", f"City {city_name.capitalize()} not found. Please try again."))
            else:
                speak("Please specify a city name.")
                ui_queue.put(("system_output", "Please specify a city name."))

        except requests.exceptions.RequestException as e:
            speak("I couldn't fetch the weather information due to a connection issue.")
            ui_queue.put(("system_output", "I couldn't fetch the weather information due to a connection issue."))

        except Exception as e:
            speak("An error occurred while fetching the weather information.")
            ui_queue.put(("system_output", "An error occurred while fetching the weather information."))
    ...
    elif "very good morning" in query:
...
## keyboard Control-----------------------------------------------
    ...
    elif "go down" in query or "down move" in query:
        try:
    ...
    elif "go up" in query or "top move" in query:
        try:
            pyautogui.press('up')
            speak("Went up.")
    ...
    elif "go left" in query or "left move" in query:
        try:
            pyautogui.press('left')
            speak("Went left.")
            ui_queue.put(("system_output", "Went left."))
        except Exception as e:
            speak("I couldn't left.")
            ui_queue.put(("system_output", "I couldn't left."))


    elif "go right" in query or "right move" in query:
        try:
            pyautogui.press('right')
            speak("Went right.")
            ui_queue.put(("system_output", "Went right."))
        except Exception as e:
            speak("I couldn't right.")
            ui_queue.put(("system_output", "I couldn't right."))
    ...
    elif "control e" in query:
        try:
            pyautogui.hotkey('ctrl', 'e')
            speak("done.")
            ui_queue.put(("system_output", "done."))
        except Exception as e:
            speak("I couldn't")
            ui_queue.put(("system_output", "I couldn't."))

    elif "control k" in query:
        try:
    ...
    # ____________________________________________Mouse Controls_________________
    ...
    elif "get cursor position" in query:
    ...
    # move Right ..................................................
        ...
        speak("Done")
        #continue
    elif "move down right side" in query:
        mouse.move(100, 100, absolute=False, duration=0.1)
        speak("Done")
        #continue
    elif "move up right side" in query:
        mouse.move(100, -100, absolute=False, duration=0.1)
        speak("Done")
        #continue
...
#        continue
        ...
        speak("Done")
 #       continue
    elif "move up left side" in query:
        mouse.move(-100, -100, absolute=False, duration=0.1)
        speak("Done")
  #      continue
    elif "move down left side" in query:
        mouse.move(-100, 100, absolute=False, duration=0.1)
        speak("Done")
    #    continue
   ...
   #     continue
    ...
    # move up and down.....................
    elif "move up" in query or "move upper side" in query:
 ...
 #       continue
  ...
  #      continue
   ...
   #     continue
    ...
    # scroll down
    elif "long scroll down" in query:
        mouse.wheel(-7)
        speak("ok")
#        continue
    # scroll up
    elif "long scroll up" in query:
        mouse.wheel(7)
        speak("Done")
 #       continue
  ...
  #      continue
        ...
        speak("ok")
   #     continue
    elif "Extra scroll down" in query:
        mouse.wheel(-3)
        speak("ok")
    #        continue
    # scroll up
    elif "Extra scroll up" in query:
        mouse.wheel(3)
        speak("Done")
    #       continue
    ...
    #----------------------------------------Some Flexibility Commands--------------
        ...
        if match:
            ...
            steps = percentage // 10  # Adjust the number of steps based on the percentage

            if 'zoom in' in query:
                for _ in range(steps):
                    pyautogui.hotkey('ctrl', '+')
                    ui_queue.put(f"Zooming in by {percentage}%.")

            elif 'zoom out' in query:
                for _ in range(steps):
                    pyautogui.hotkey('ctrl', '-')
                    ui_queue.put(f"Zooming out by {percentage}%.")
        else:
            ui_queue.put("Please specify a percentage for zooming.")
        # Example usage within the command processing
    ...
    elif 'volume up to' in query or 'volume down to' in query or 'set volume' in query:
    ...
    elif 'brightness to' in query or 'set brightness' in query:
...
#-----------------------------------Integrations------------------------------------
    elif 'activate screen keyboard' in query or 'activate on screen keybpard' in query or 'activate keyboard' in query:
        if __name__ == "__main__":
            root = tk.Tk()
            app = ProfessionalKeyboard(root)
            root.mainloop()

 #   elif 'activate mouse' in query or 'activate virtual mouse' in query:

        # ---------------------------------Integrations--------------------------------------
    elif 'show my files' in query or 'from my files find' in query:
        try:
            if 'from my files find' in query:
                folder_name = query.replace("from my files find", "").strip()
            else:
                folder_name = ""  # Set a default for when just "show my files" is called

            if folder_name:
                speak(f"Searching for the directory named {folder_name} across all drives...")
                ui_queue.put(("system_output",
                              f"Searching for the directory named {folder_name} across all drives..."))  # Update UI

                # Perform the search
                folder_path = search_directory(folder_name)

                if folder_path:
                    try:
                        open_directory(folder_path)
                        speak(f"Opened the directory: {folder_name}")
                        ui_queue.put(("system_output", f"Opened the directory: {folder_path}"))  # Update UI
                    except Exception as e:
                        speak(f"Unable to open the directory. Error: {str(e)}")
                        ui_queue.put(("system_output", f"Error: Unable to open the directory. {str(e)}"))  # Update UI
                else:
                    speak(f"Sorry, I couldn't find a directory named {folder_name}.")
                    ui_queue.put(("system_output", f"Directory named {folder_name} not found."))  # Update UI
            else:
                speak("Please provide a folder name to search.")
                ui_queue.put(("system_output", "No folder name provided for search."))  # Unclear input update to UI

        except Exception as e:
            speak(f"An error occurred: {str(e)}")
            ui_queue.put(("system_output", f"Error: {str(e)}"))  # Update UI with error
    ...
    speak(f"Total response time for the command was {response_time:.2f} seconds.")
    ui_queue.put(("system_output", f"Total response time: {response_time:.2f} seconds."))"""
# Initialize Bot Name
bot_name = "Misty"
